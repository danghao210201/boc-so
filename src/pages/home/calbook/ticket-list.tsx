...
function TicketList() {
  ...
  const fetchTicketData = async () => {
    ...
    try {
      ...
      const servingResponse = await fetch(`https://demo8.tayninh.gov.vn/backend/api/screen/serving.php?dept_id=23&counter_id=${selectedCounter.id}`);
      const servingData: ServingResponse = await servingResponse.json();

      // Gọi API đang chờ với filter theo quầy
      const upcomingResponse = await fetch(`https://demo8.tayninh.gov.vn/backend/api/screen/upcoming.php?dept_id=23&counter_id=${selectedCounter.id}`);
